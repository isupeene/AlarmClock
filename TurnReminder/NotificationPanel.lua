...
--  Notification Panel  
-- ===========================================================================
print("Loading NotificationPanel.lua");
...
local pNotificationTable        : table = {};               -- Custom Notiftable
...
function RegisterHandlers()
    ...
    g_notificationHandlers[888888]                                                  = MakeDefaultHandlers();

    g_notificationHandlers[888889]                                                  = MakeDefaultHandlers();
    g_notificationHandlers[888889].Activate                                        = OnCustomNotificationActivate;
  ...
  if notificationEntry ~= nil then
    ...
    if table.count(notificationEntry.m_IDs) == 0 then
      ...
      local pNotification:table = NotificationManager.Find( playerID, nextID );
            if not pNotification  then
                local kNotificationCustom = playerID.."."..nextID
                pNotification = pNotificationTable[kNotificationCustom]
            end
  ...
  for _,kNotification in pairs(kPlayerTable) do
    for _,id in ipairs(kNotification.m_IDs) do
      pNotification = NotificationManager.Find( playerID, id );
            if not pNotification  then
                local kNotificationCustom = playerID.."."..id
                pNotification = pNotificationTable[kNotificationCustom]
            end
      if (pNotification ~= nil) then
        if ( topNotification.m_TypeName == pNotification:GetTypeName() ) and ( pNotification:CanUserDismiss() ) then
          NotificationManager.Dismiss( pNotification:GetPlayerID(), pNotification:GetID() );
                    if pNotification.IsCustom then
                        OnNotificationDismissed( pNotification:GetPlayerID(), pNotification:GetID() )  
                        local kNotificationCustom = pNotification:GetPlayerID().."."..pNotification:GetID()
                        pNotificationTable[kNotificationCustom] = nil   
                    end
        end
      end
    end
...
function OnDefaultAddNotification( pNotification:table )
  ...
  local notificationID    :number        = pNotification:GetID();
   ...
   local notificationPrimaryIconName:string    = pNotification:GetIconName();
    local isCustom                                  = pNotification.IsCustom  
  local notificationEntry    :NotificationType  = AddNotificationEntry(playerID, typeName, notificationID, notificationGroupID, notificationPrimaryIconName);
    if isCustom then
        local kNotificationCustom = playerID.."."..notificationID
        if pNotificationTable[kNotificationCustom] == nil then
            pNotificationTable[kNotificationCustom] = pNotification
        end
    end
  if (notificationEntry == nil) then
...
function RealizeStandardNotification( playerID:number, notificationID:number )
  ...
  if count > 1 then    
    ...
    local pNotificationCurrentIndex = NotificationManager.Find(playerID, notificationEntry.m_IDs[ notificationEntry.m_Index ])
    -- Get CustomNotif if nil
    if not pNotificationCurrentIndex then
            local kNotificationCustom = playerID.."."..notificationEntry.m_IDs[ notificationEntry.m_Index ]
            pNotificationCurrentIndex = pNotificationTable[kNotificationCustom]
    end
  -- Set text again now that calculations are done, text must always match the current index!!
  SetNotificationText(notificationEntry, pNotificationCurrentIndex);
...
function OnDefaultNextSelectNotification( pNotification:table )
  ...
  local pNextNotification    :table = NotificationManager.Find( playerID, nextID );
    if not pNextNotification then
        local kNotificationCustom = playerID.."."..nextID
        pNextNotification = pNotificationTable[kNotificationCustom]
    end
...
-- ===========================================================================
--  Remove Custom notification On Turn Start
-- ===========================================================================

function OnPlayerTurnStartNotificationDismis(PlayerID, firstTime)
    --local previousPlayerID = Game.GetLocalPlayer()-1 < 0 and 63 or Game.GetLocalPlayer()-1
    if firstTime and PlayerID == Game.GetLocalPlayer() then
        for k,pNotification in pairs (pNotificationTable) do   
            if pNotification:GetPlayerID() == Game.GetLocalPlayer() and pNotification:GetAddTurn() < Game.GetCurrentGameTurn() and pNotification.EraseOnStartTurn then
                OnNotificationDismissed( pNotification:GetPlayerID(), pNotification:GetID() )
...
function OnCustomNotificationActivate(notificationEntry)
    if (notificationEntry ~= nil and notificationEntry.m_PlayerID == Game.GetLocalPlayer()) then
    local pNotification :table = GetActiveNotificationFromEntry(notificationEntry);
    if pNotification ~= nil then
            LookAtNotification( pNotification );
      LuaEvents[pNotification.GetEventOnActivate](pNotification)
            if pNotification.DissmissOnActivate then
                OnNotificationDismissed( pNotification:GetPlayerID(), pNotification:GetID() ) 
            end
    end 
    end
...
function LateInitialize()
  ...
  LuaEvents.ActionPanel_ActivateNotification.Add( OnLuaActivateNotification );
    ...
    -- notif mod
    LuaEvents.CustomNotification_OnDefaultAddNotification.Add( OnDefaultAddNotification );
    Events.PlayerTurnActivated.Add( OnPlayerTurnStartNotificationDismis)    -- should not crash the game?
end
